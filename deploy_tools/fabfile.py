...
REPO_URL = 'https://github.com/Donnyvdm/superlists.git'
...
def _update_static_files(source_folder):
    run('cd {0} && ../virtualenv/bin/python manage.py collectstatic --noinput'.format(
        source_folder
    ))

def _update_database(source_folder):
    run('cd {0} && ../virtualenv/bin/python manage.py migrate --noinput'.format(
        source_folder
    ))
